...
bash scripts/create_traj.sh --game "$game" --init_state_group "$init_state_group" --init_states "$init_states" --run_name "$run_name" --sweep true


bash scripts/group_trajectories.sh --game "$game" --replay_buffer_folder $run_name/ --save_path $storage_dir/grouped_trajectories/$game/$run_name/ --z_min $z_min
