...
def save(obj, push_to_hub):
    ...
    if not os.path.exists(trajectory_path):
        log_error(
            f"Trajectory path {trajectory_path} does not exist.", parameters=parameters
        )
    ...
    if os.path.exists(data_dir):
        if obj["overwrite"]:
            ...
            log_error(
                f"{data_dir} already exists. Use --overwrite to overwrite.",
    ...
    if os.path.exists(df_out_path):
        if obj["overwrite"]:
            ...
            log_error(
                f"{df_out_path} already exists. Use --overwrite to overwrite.",
                parameters=parameters,
            )
    log_info(
        f"Saving annotations from {out_path} and saving to {df_out_path}. Images will be saved to {image_dir}",
        parameters=parameters,
    )
    columns = [
        "group_idx",
    ...
    for group_idx in tqdm(
        group_idxes, desc="Processing groups", total=len(group_idxes)
    ):
        all_steps = annotations[group_idx]  # list
        for step in tqdm(
            all_steps, desc="Processing steps", total=len(all_steps), leave=False
        ):
            if not step["good_action"]:
                continue
            traj_idx = step["traj_idx"]
            observations, actions, high_level_actions, rewards = grouped_trajectories[
                int(group_idx)
            ][traj_idx]
            step_idx = step["step"]
    ...
    log_info(
        f"Saved dataset with {len(data)} rows to {df_out_path}", parameters=parameters
    )
    if push_to_hub:
        # TODO
        pass
