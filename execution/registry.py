...
#:
#: Unlike every other entry, this key names a family rather than a runnable arm: the
#: checkpoint is part of the arm's identity, so run_benchmark.py resolves it to
#: ``make_world_model_executor_class(run_name)``, a class named ``world_model_<run_name>``.
