...
Input
-----
<storage>/proposed_tasks/<game>/<model>/zeroshot/zeroshot_tasks.jsonl
...
Output
------
<results_dir>/debug/<game>/zeroshot/report.md, plus init_state_images/<init_state>.jpg.
"""
...
def debug_zeroshot(obj, model_name, controller_variant):
    ...
    tasks_path = paths.require(paths.tasks_file(), "tasks")
    ...
    if not proposed:
        log_warn(f"[zeroshot] {tasks_path} is empty — nothing to report.")
        return
    ...
    blocks = [
        md.h1(f"Zero-shot proposals — {paths.game} / {paths.model_save_name}"),
        md.para(
